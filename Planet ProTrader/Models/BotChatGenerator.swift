...
class BotChatEngine: ObservableObject {
    ...
    private func generateMessageContent(for bot: BotPersona, in channel: ProTraderChannel) -> String {
        ...
        content = content.replacingOccurrences(of: "{price}", with: String(format: "$%.2f", Double.random(in: 2300...2400)))
        content = content.replacingOccurrences(of: "{percentage}", with: String(format: "%.1f%%", Double.random(in: -5...5)))
    ...
    private func getTrashTalkLevel(for bot: BotPersona) -> Double {
        ...
        case .brutal: return Double.random(in: 0.8...1.0)
