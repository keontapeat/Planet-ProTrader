...
class MultiAIIntegration: ObservableObject {
    ...
    func detectPatterns(in data: [GoldDataPoint]) async -> [AITradingPattern] {
        let prompt = """
        ...
        var allPatterns: [AITradingPattern] = []
    ...
    private func validatePatternsWithConsensus(_ patterns: [AITradingPattern]) -> [AITradingPattern] {
        var validatedPatterns: [AITradingPattern] = []
...
class OpenAIModel: AIModel {
    ...
    func analyzeMarket(prompt: String, marketData: TradingModels.MarketData) async -> AIResponse? {
        ...
        let goldMarketData = TradingTypes.GoldMarketData(
            trend: marketData.changePercentage > 0 ? SharedTypes.MarketTrend.bullish : SharedTypes.MarketTrend.bearish,
