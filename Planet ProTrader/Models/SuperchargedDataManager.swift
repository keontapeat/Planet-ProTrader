...
class SuperchargedDataManager: ObservableObject {
    ...
    func getHighProbabilitySignals() -> [SharedTypes.TradingOpportunity] {
        var opportunities: [SharedTypes.TradingOpportunity] = []
        ...
        if sentimentScore > 1.1 && hasEconomicEvents && socialAligned {
            let direction: SharedTypes.TradeDirection = goldSentiment == .strongBullish || goldSentiment == .bullish ? .buy : .sell
            let targetPrice = direction == .buy ? currentGoldPrice + 30 : currentGoldPrice - 30
            let stopLoss = direction == .buy ? currentGoldPrice - 15 : currentGoldPrice + 15
            
            let opportunity = SharedTypes.TradingOpportunity(
                symbol: "XAUUSD",
                direction: direction,
