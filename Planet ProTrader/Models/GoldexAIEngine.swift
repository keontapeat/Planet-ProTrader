...
// MARK: - Planet ProTrader AI Engine

@MainActor
class PlanetProTraderAIEngine: ObservableObject {
    ...
    // MARK: - Basic Properties for HomeView compatibility
    @Published var isActive: Bool = true
    @Published var todaysSignals: Int = 12
    @Published var accuracy: Double = 87.5
    @Published var currentStatus: String = "AI Engine Active - Analyzing Markets"
    @Published var marketSentiment: String = "Bullish"
    @Published var confidence: Double = 85.0
    ...
    private var performanceMetrics: [PlanetProTraderPerformanceMetric] = []
    ...
    func updateStatus() async {
        // Update AI status with realistic simulation
        isActive = Bool.random() ? true : true // Keep mostly active
        todaysSignals = Int.random(in: 8...15)
        accuracy = Double.random(in: 82...92)
        confidence = Double.random(in: 80...95)
        
        let statuses = [
            "AI Engine Active - Analyzing Markets",
        ...
        currentStatus = statuses.randomElement() ?? "AI Engine Active"
        
        let sentiments = ["Bullish", "Bearish", "Neutral"]
        marketSentiment = sentiments.randomElement() ?? "Neutral"
...
struct PlanetProTraderPerformanceMetric {
    let timestamp: Date
    let winRate: Double
...
#Preview {
    VStack {
        Text("Planet ProTrader AI Engine")
            .font(.title)
            .padding()
        ...
        Text("Quantum-Enhanced Trading Intelligence")
            .font(.caption)
...
typealias GoldexAIEngine = PlanetProTraderAIEngine
