...
#if DEBUG
struct BackgroundSystemsModifierPreview: View {
    @StateObject private var systemMonitor = SystemMonitor()
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack {
                    Image(systemName: systemMonitor.systemHealth.level.systemImage)
                        .foregroundColor(systemMonitor.systemHealth.level.color)
                        .font(.title2)
                    
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Health Score: \(systemMonitor.systemHealth.formattedHealthScore)")
                            .font(.subheadline)
                            .fontWeight(.medium)
                        
                        Text("Memory: \(String(format: "%.1f%%", systemMonitor.systemHealth.memoryUsage * 100))")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        
                        Text("CPU: \(String(format: "%.1f%%", systemMonitor.systemHealth.cpuUsage * 100))")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    ...
                    Text(systemMonitor.isMonitoring ? "Active" : "Inactive")
                        .font(.subheadline)
                        .fontWeight(.medium)
