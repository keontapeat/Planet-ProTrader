...
final class SupabaseService: ObservableObject {
    ...
    private func setupRealtimeSubscriptions() {
        ...
        Task {
            ...
            await tradesChannel
                .onInsert { [weak self] insert in
                    Task { @MainActor in
                        await self?.handleNewTrade(insert)
                    }
                }
                .subscribe()
        }
    ...
    private func handleNewTrade(_ insert: RealtimeInsertPayload) async {
    ...
    func saveHistoricalDataBatch(data: [EnhancedGoldDataPoint], botId: UUID) async throws {
        ...
        for batch in batches {
            let batchData = batch.map { dataPoint in
                return [
                    "bot_id": botId.uuidString,
