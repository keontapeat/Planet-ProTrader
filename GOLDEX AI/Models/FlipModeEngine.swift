...
class FlipModeEngine: ObservableObject {
        ...
        do {
            ...
            if success {
                ...
                if let vpsIndex = vpsConnections.firstIndex(where: { $0.id == vps.id }) {
                    vpsConnections[vpsIndex].accountsRunning += 1
                }
    ...
    private func executeFlipOnAccount(_ account: SharedTypes.DemoAccount, strategy: SharedTypes.FlipStrategy) async {
        while isFlipModeActive && account.status == .active {
    ...
    private func executeFlipTrade(account: SharedTypes.DemoAccount, signal: SharedTypes.FlipSignal) async {
        do {
            ...
            let beforeScreenshot = await screenshotEngine.captureChart(
                vpsId: account.vpsIndex.description,
...
// MARK: - Extensions for Demo Account

extension SharedTypes.DemoAccount {
    ...
    var vpsIndex: Int {
        return Int(id.hashValue) % 10 // Distribute across 10 VPS instances
    }
...
// MARK: - Flip Strategy Extensions

extension SharedTypes.FlipStrategy {
    var timeframes: [String] {
        switch self {
        case .scalping: return ["1M", "5M"]
        case .dayTrading: return ["15M", "1H"]
    ...
    var mode: FlipStrategyMode {
        switch self {
        case .scalping: return .aggressive
        case .dayTrading: return .balanced
        case .swing: return .conservative
        default: return .balanced
        }
    }
    
    enum FlipStrategyMode {
        case conservative
        case balanced
        case aggressive
        case extreme
