...
enum NewsImpact: String, CaseIterable {
    ...
    case volatile = "High Volatility Expected"
}
...
class GPTIntegration: ObservableObject {
    ...
    func analyzeMarketConditions(currentPrice: Double, marketData: TradingTypes.GoldMarketData) async -> GPTMarketAnalysis? {
        ...
        let prompt = """
        🏛️ GOLDEX GPT-4 TRADING ORACLE 🏛️
    ...
    private func extractImpact(from content: String) -> NewsImpact {
        let lowercased = content.lowercased()
        if lowercased.contains("bullish") {
            return .bullish
        } else if lowercased.contains("bearish") {
            return .bearish
        } else if lowercased.contains("volatile") {
            return .volatile
        } else {
            return .neutral
        }
