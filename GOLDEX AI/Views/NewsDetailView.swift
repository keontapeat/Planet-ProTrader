...
struct NewsDetailView: View {
    ...
    private var articleHeader: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                ...
                Text(article.timestamp.formatted(.dateTime.weekday().month().day().hour().minute()))
                    .font(.caption)
    ...
    private var impactMeter: some View {
        VStack(spacing: 8) {
            HStack {
                ...
                Text(article.impact.displayName.uppercased())
                    .font(.caption)
                    .fontWeight(.bold)
    ...
    private var articleContent: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("This would contain the full article content from the news provider's API. In a real implementation, this would show the complete article text with proper formatting and any embedded media.")
                    .font(.body)
                    .foregroundColor(.secondary)
                    .padding()
                    .background(Color(.systemGray6).opacity(0.5))
    ...
    private var currencyImpactSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 3), spacing: 8) {
                ForEach(article.affectedCurrencies, id: \.self) { currency in
    ...
    private func relatedNewsCard(article: NewsArticle) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(article.title)
                .font(.caption)
                .fontWeight(.semibold)
    ...
    private func impactBadge(impact: NewsImpact) -> some View {
        HStack(spacing: 4) {
    ...
    private func sentimentBadge(sentiment: NewsSentiment) -> some View {
        HStack(spacing: 4) {
...
class NewsMarketAnalyzer: ObservableObject {
    ...
    private func performAnalysis(_ article: NewsArticle) {
        ...
        volatilityImpact = article.impact == .high ? "Very High" : "Moderate"
        analysisConfidence = Double.random(in: 0.75...0.95)
        urgencyLevel = article.impact == .high ? "High" : "Medium"
    }
    
    private func generateImplications(for article: NewsArticle) -> [String] {
        return [
            "Increased volatility expected in \(article.affectedCurrencies.joined(separator: ", ")) pairs",
    ...
    private func generateRelatedArticles(for article: NewsArticle) -> [NewsArticle] {
        // Return sample related articles
        return Array(MarketNewsManager.shared.news.filter { $0.id != article.id }.prefix(3))
    }
    
    private func generateTradingOpportunities(for article: NewsArticle) -> [TradingOpportunity] {
        let currencies = article.affectedCurrencies
