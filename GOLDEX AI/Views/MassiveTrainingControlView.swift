...
struct MassiveTrainingControlView: View {
    ...
    private func resultsSection(_ results: MassiveTrainingResults) -> some View {
        VStack(spacing: 16) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 12) {
                ResultCard(title: "Bots Trained", value: "\(results.trainedBots)", color: .blue)
                ResultCard(title: "Data Points", value: "\(results.totalDataPoints)", color: .green)
                ResultCard(title: "GODMODE Bots", value: "\(results.newGodmodeBots)", color: .gold)
                ResultCard(title: "Training Time", value: "\(String(format: "%.1f", results.totalTrainingTime))s", color: .purple)
            }
