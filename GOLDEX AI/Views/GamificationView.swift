...
struct GamificationView: View {
    ...
    private var quickStatsSection: some View {
        LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 4), spacing: 12) {
            StatCard(
                title: "Win Streak",
                value: "\(gameEngine.tradingStreaks.profitStreak)",
                icon: "flame.fill",
                color: .red
            )
            ...
            StatCard(
                title: "Daily Streak",
                value: "\(gameEngine.challengeStreak)",
            ...
            StatCard(
                title: "Total XP",
                value: "\(gameEngine.totalXPEarned.formatted())",
                icon: "star.fill",
                color: .gold
            )
            ...
            StatCard(
                title: "Achievements",
                value: "\(gameEngine.unlockedAchievements.count)/\(gameEngine.availableAchievements.count)",
                icon: "trophy.fill",
                color: .purple
            )
        }
            ...
            if gameEngine.pendingRewards.isEmpty {
                ...
                LazyVStack(spacing: 8) {
                    ForEach(gameEngine.pendingRewards.prefix(5)) { reward in
                        PendingRewardCard(reward: reward) {
                            claimReward(reward)
                        }
                    }
                }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            LazyVStack(spacing: 12) {
                ForEach(gameEngine.dailyChallenges) { challenge in
                    DailyChallengeCard(challenge: challenge, gameEngine: gameEngine)
                }
            }
        }
        .padding()
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            LazyVStack(spacing: 12) {
                ForEach(gameEngine.weeklyChallenges) { challenge in
                    WeeklyChallengeCard(challenge: challenge, gameEngine: gameEngine)
                }
            }
        }
        .padding()
    ...
    private func activeBossBattleSection(_ boss: BossBattle) -> some View {
        VStack(spacing: 20) {
            ...
            BossBattleCard(boss: boss, gameEngine: gameEngine)
        }
        .padding()
        .background(
            RoundedRectangle(cornerRadius: 16)
    ...
    private var guildContributionsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 12) {
                ContributionCard(
                    title: "Weekly XP",
                    value: "\(gameEngine.guildContributions.weeklyXP)",
                    icon: "star.fill",
                    color: .cyan
                )
                ...
                ContributionCard(
                    title: "Trades",
                    value: "\(gameEngine.guildContributions.weeklyTrades)",
                ...
                ContributionCard(
                    title: "Profit",
                    value: "$\(String(format: "%.0f", gameEngine.guildContributions.weeklyProfit))",
                ...
                ContributionCard(
                    title: "Guild Rank",
                    value: "#\(gameEngine.guildContributions.rank)",
                    icon: "crown.fill",
                    color: .purple
                )
            }
