...
class MarketImpactAnalyzer: ObservableObject {
    ...
    func analyzeMarketImpact(for article: NewsArticle) async {
    ...
    private func performAnalysis(for article: NewsArticle) {
        ...
        generateCurrencyImpacts(for: article)
    ...
    private func calculateImpactScore(for article: NewsArticle) -> Double {
        var score = 50.0
        ...
        case .bullish, .bearish: score += 15
        }
        
        score += Double(article.affectedCurrencies.count) * 5
    ...
    private func generateCurrencyImpacts(for article: NewsArticle) {
        currencyImpacts = article.affectedCurrencies.map { currency in
    ...
    private func generateAIPredictions(for article: NewsArticle) {
        aiPredictions = [
            AIPrediction(
    ...
    private func setRiskLevels(for article: NewsArticle) {
        volatilityRisk = article.impact == .high ? .high : .medium
        liquidityRisk = article.affectedCurrencies.count > 3 ? .medium : .low
...
#Preview {
    MarketImpactAnalysisView(article: NewsArticle(
