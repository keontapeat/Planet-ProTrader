...
class NotificationCenter {
    static let `default` = Foundation.NotificationCenter.default
    ...
    @discardableResult
    static func observeAccountBalanceUpdates(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
            forName: .accountBalanceUpdated,
            object: nil,
            queue: .main,
            using: { notification in
                (observer as? NSObject)?.perform(selector, with: notification)
            }
        )
    }
    
    @discardableResult
    static func observeTradeExecutions(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
            forName: .tradeExecuted,
            object: nil,
            queue: .main,
            using: { notification in
                (observer as? NSObject)?.perform(selector, with: notification)
    ...
    @discardableResult
    static func observeEAStatusChanges(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
            forName: .eaStatusChanged,
            object: nil,
            queue: .main,
            using: { notification in
                (observer as? NSObject)?.perform(selector, with: notification)
    ...
    @discardableResult
    static func observeSignalGeneration(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
            forName: .signalGenerated,
            object: nil,
            queue: .main,
            using: { notification in
                (observer as? NSObject)?.perform(selector, with: notification)
    ...
    @discardableResult
    static func observeDebugSessions(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
            forName: .debugSessionCompleted,
            object: nil,
            queue: .main,
            using: { notification in
                (observer as? NSObject)?.perform(selector, with: notification)
    ...
    @discardableResult
    static func observePriceUpdates(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
            forName: .priceUpdated,
            object: nil,
            queue: .main,
            using: { notification in
                (observer as? NSObject)?.perform(selector, with: notification)
    ...
    @discardableResult
    static func observeSystemHealthChanges(observer: Any, selector: Selector) -> NSObjectProtocol {
        return default.addObserver(
    ...
    static func removeObserver(_ observer: Any, name: Notification.Name) {
        default.removeObserver(observer, name: name, object: nil)
    }
}
...
struct NotificationExtensionsPreview: View {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 12) {
                    Button("Post Account Balance Update") {
                        NotificationCenter.postAccountBalanceUpdate(balance: 1270.85, account: "Demo-845060")
                        notifications.append("Account balance updated: $1270.85")
                    }
                    .goldexButtonStyle()
                    
                    Button("Post Trade Executed") {
                        NotificationCenter.postTradeExecuted(trade: "XAUUSD Buy", profit: 45.50)
                        notifications.append("Trade executed: +$45.50 profit")
                    }
                    .goldexButtonStyle()
                    
                    Button("Post Signal Generated") {
                        NotificationCenter.postSignalGenerated(signal: "XAUUSD Signal", confidence: 0.89)
                        notifications.append("Signal generated: 89% confidence")
                    }
                    .goldexButtonStyle()
                    
                    Button("Post EA Status Change") {
                        NotificationCenter.postEAStatusChanged(isRunning: true, canTrade: true)
    ...
    private func setupNotificationObservers() {
        // This would normally set up observers for notifications
        // For demo purposes, we'll just show the posting functionality
    }
}
